...
export function buildMenuFaq(): string {
  return (
    ...
    `<b>Q. 얼마나 걸리나요?</b>\n` +
    `A. 온페이지 1~2주 / 백링크는 매월 꾸준히 / 키워드 진입은 사이트·키워드에 따라 편차 큼(1주~수개월), 꾸준한 구축이 핵심\n\n` +
    `<b>Q. 직접 적용해도 되나요?</b>\n` +
...
export function buildPrioritizedActions(report: Record<string, unknown>): PriorityAction[] {
  ...
  const backlinkAction: PriorityAction =
    myBacklinks !== null && myBacklinks < 30
      ? {
          title: '백링크 정기 구축 시작 (시급)',
          period: '월 예산 맞춤 · 매월 꾸준히',
